...
class Muzyka_Translate_Writer_Csv implements Muzyka_Translate_Writer_Interface
{
  const STATE_ADDED = 1;
  ...
  public function read($file = null)
  {
    if( null === $file && null === $this->_file ) {
      throw new Muzyka_Translate_Writer_Exception('no file to write to specified.');
    }
    ...
    $this->_file = $file;
    $tmp = Muzyka_Translate_Parser_Csv::parse($file, 'null', $this->_options);
  ...
  public function write($file = null)
  {
    ...
    if( !$fh ) {
      throw new Muzyka_Translate_Writer_Exception('unable to open file');
    }
  ...
  protected function _checkFile($file)
  {
    if( pathinfo($file, PATHINFO_EXTENSION) != 'csv' ) {
      throw new Muzyka_Translate_Writer_Exception(sprintf('file "%s" not a csv file', $file));
    }

    if( !file_exists($file) ) {
      throw new Muzyka_Translate_Writer_Exception(sprintf('file "%s" does not exist', $file));
    }

    if( !is_writeable($file) ) {
      throw new Muzyka_Translate_Writer_Exception(sprintf('file "%s" not writeable', $file));
    }
  }
