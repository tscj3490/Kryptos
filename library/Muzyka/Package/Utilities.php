...
class Muzyka_Package_Utilities
{
  // FTP Helpers
   ...
   * @param array $options
   * @return Net_FTP
   * @throws Muzyka_Package_Exception
   */
  static public function ftpFactory($options)
  {
    ...
    if( empty($host) || empty($username) || empty($password) ) {
      throw new Muzyka_Package_Exception('Partial data');
    }
    ...
    if( $ftp->isError($ret) ) {
      throw new Muzyka_Package_Exception('Unable to connect to FTP server');
    }

    // Login
    $ret = $ftp->login($username, $password);
    if( $ftp->isError($ret) ) {
      throw new Muzyka_Package_Exception('Unable to login to FTP server');
    }
  ...
  static public function ftpSearch(Net_FTP $ftp, $pattern, $path = '/', $stopOnMatch = false)
  {
    ...
    if( !is_array($pattern) && !is_string($pattern) ) {
      throw new Muzyka_Package_Exception('invalid pattern');
    }
    $type = ( is_array($pattern) ? 'list' : ( $pattern[0] == '/' ? 'regex' : 'name' ) );

    // Check path
    if( empty($path) ) {
      throw new Muzyka_Package_Exception('invalid path');
    }

    // Change dir
    $ret = $ftp->cd($path);
    if( $ftp->isError($ret) ) {
      throw new Zend_Exception($ret->getMessage(), $ret->getCode());
    }
  ...
  static public function ftpLsRecursive(Net_FTP $ftp, $dir, $mode = null)
  {
    ...
    $ret = $ftp->ls($dir, $mode);
    if( PEAR::isError($ret) ) {
      throw new Muzyka_Package_Exception($ret->getMessage());
    }
  ...
  static public function ftpInclude()
  {
    ...
    if( !function_exists('ftp_connect') ) {
      ...
      if( !function_exists('ftp_connect') ) {
        throw new Muzyka_Package_Exception('Unable to load ftp emulation layer.');
      }
    }
    if( !class_exists('Net_FTP', false) ) {
      include_once 'Net/FTP.php';
      if( !class_exists('Net_FTP', false) ) {
        throw new Muzyka_Package_Exception('Unable to load class Net_FTP.');
      }
    }
  }
    ...
    foreach( $it as $item ) {
      ...
      if( $item->isDir() ) {
        ...
        if( !copy($item->getPathname(), $fDest) ) {
          throw new Muzyka_Package_Exception('Unable to copy.');
        }
      }
    }
  }
  ...
  static public function fsRmdirRecursive($path, $includeSelf = false)
  {
    ...
    foreach( $it as $key => $child ) {
      ...
      if( $it->isDir() ) {
        if( !rmdir($key) ) {
          throw new Muzyka_Package_Exception(sprintf('Unable to remove directory: %s', $key));
        }
      } else if( $it->isFile() ) {
        if( !unlink($key) ) {
          throw new Muzyka_Package_Exception(sprintf('Unable to remove file: %s', $key));
        }
      }
    }
    ...
    if( is_dir($path) && $includeSelf ) {
      if( !rmdir($path) ) {
        throw new Muzyka_Package_Exception(sprintf('Unable to remove directory: %s', $path));
      }
    }
  }
